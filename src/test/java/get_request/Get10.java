...
public class Get10 extends GoRestBaseUrl {
    ...
    public void get10(){
        ...
        Map<String,Object> actualData = response.as(HashMap.class);// Json response to Java map -- deserialization
        System.out.println("actualData = " + actualData);
        assertEquals(expectedData.get("meta"),actualData.get("meta"));
        assertEquals(expectedData.get("meta"),actualData.get("meta"));
      //  assertEquals(dataKeyMap.get("name"), ((Map)actualData.get("data")).get("name"));
        assertEquals(dataKeyMap.get("name"), json.getString("data.name"));
